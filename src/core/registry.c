    ...
    void *handle)
{
    ...
    case CLAW_MOD_PROVIDER:
        if (!desc->api.provider || !desc->api.provider->name || !desc->api.provider->chat) {
            return -6;
        }
    ...
    case CLAW_MOD_MEMORY:
        if (!desc->api.memory || !desc->api.memory->name || !desc->api.memory->put || !desc->api.memory->get) {
            return -6;
        }
...
const claw_provider_api_t *
claw_registry_get_provider(const claw_registry_t *r, const char *name)
{
    size_t i;
    if (!r || !name) return NULL;

    for (i = 0; i < r->provider_count; ++i) {
        if (r->providers[i] && r->providers[i]->name &&
            strcmp(r->providers[i]->name, name) == 0) {
...
const claw_memory_api_t *
claw_registry_get_memory(const claw_registry_t *r, const char *name)
{
    size_t i;
    if (!r || !name) return NULL;

    for (i = 0; i < r->memory_count; ++i) {
        if (r->memories[i] && r->memories[i]->name &&
            strcmp(r->memories[i]->name, name) == 0) {
            return r->memories[i];
        }
    }
    return NULL;
